...
router.post("/", async (req, res) => {
    const { description, taskId, ownerId } = req.body;
    const values = [description, taskId, ownerId];
    try {
        const insertQuery = `INSERT INTO "COMMENT" ( "DESCRIPTION", "TASK_ID", "OWNER_ID") VALUES ($1,$2,$3) RETURNING *`;
        const result = await db.query(insertQuery, values);
        res.status(201).json(result.rows[0]);
    } catch (error) {
        console.error("코멘트 생성 중 오류 발생: ", error);
        res.status(500).json({ error: "코멘트 생성 중 오류 발생" });
    }
});
